...
namespace innerproducts {
    ...
    */
    ...
    struct GaussHermiteQR
    {
        ...
        using NodeMatrix = Eigen::Matrix<real_t,1,Eigen::Dynamic>;   // Node, sizes (1*|R|), where |R| is the number of nodes 
        using WeightVector = Eigen::Matrix<real_t,1,Eigen::Dynamic>; // Weight, sizes (1*|R|), where |R| is the number of nodes 
