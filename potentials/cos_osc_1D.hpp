...
{
    /**
    * \brief This class represents 1D cosin oscillator V(x)=a(-cos(bx)+1) 
    *
    * \tparam D dimensionality of 1D cosin oscillator (number of variables)
    */
	...
	struct Cos_osc_1D
	{
        ...
        Cos_osc_1D()= default;  // default constructor 

        Cos_osc_1D(real_t a, real_t b, const RMatrixDD &mass_inv): a_(a), b_(b), mass_inv_(mass_inv) {}  //constructor 

        //copy constructor 
        Cos_osc_1D(const Cos_osc_1D &that){
        ...
        //assignment 
        Cos_osc_1D &operator=(const Cos_osc_1D& that){
        ...
        /**
        * \brief calculate the value of 1D cosin oscillator at position pos
        * \param[in] pos  the position at which we calculate the value of 1D cosin oscillator
        */

    	real_t evaluate_pes(const RMatrixD1& pos) const 
    	{
            return a_*(-std::cos(b_*pos(0))+1.0);
    	}



        /**
        * \brief calculate the gradient of 1D cosin oscillator at position pos 
        * \param[in] pos the position at which we calculate the gradient of 1D cosin oscillator
        */
    	...
    	RMatrixD1 evaluate_grad(const RMatrixD1& pos) const 
    	{
            ...
            grad(0)=a_*b_*std::sin(b_*pos(0));

            return grad;
    	}

        /**
        * \brief calculate the hessian of 1D cosin oscillator at position pos 
        * \param[in] pos the position at which we calculate the 1D cosin oscillator
        */
    	...
    	RMatrixDD evaluate_hess(const RMatrixD1& pos) const 
    	{
            ...
            hess(0,0)=a_*b_*b_*std::cos(b_*pos(0));
    ...
    private:
        ...
        complex_t evaluate_pes_c(const CMatrixD1 &pos) const{
            ...
            return complex_t(a_*(-std::cos(b_*pos(0).real())+1.0),0.0);
        }
