...
{
    /**
    * \brief This class represents 2D cosin oscillator V(x)=-cos(ax)*cos(by)
    *
    * \tparam D dimensionality of 2D cosin oscillator (number of variables)
    */

	template<dim_t D>
	struct Cos_osc_mul_2D
	{
        ...
        Cos_osc_mul_2D()= default;  // default constructor 

        Cos_osc_mul_2D(real_t a, real_t b, const RMatrixDD &mass_inv): a_(a), b_(b), mass_inv_(mass_inv) {}  //constructor 

        //copy constructor 
        Cos_osc_mul_2D(const Cos_osc_mul_2D &that){
        ...
        //assignment 
        Cos_osc_mul_2D &operator=(const Cos_osc_mul_2D& that){
        ...
        */
    	...
    	{
            return -std::cos(a_*pos(0))*std::cos(b_*pos(1));
    	}
        ...
        */
    	...
    	RMatrixD1 evaluate_grad(const RMatrixD1& pos) const 
    	{
            ...
            grad(0)=a_*std::sin(a_*pos(0))*std::cos(b_*pos(1));
            grad(1)=b_*std::cos(a_*pos(0))*std::sin(b_*pos(1));
        ...
        */
    	...
    	RMatrixDD evaluate_hess(const RMatrixD1& pos) const 
    	{
            ...
            hess(0,0)=a_*a_*std::cos(a_*pos(0))*std::cos(b_*pos(1));
            hess(0,1)=-a_*b_*std::sin(a_*pos(0))*std::sin(b_*pos(1));
            hess(1,0)=-a_*b_*std::sin(a_*pos(0))*std::sin(b_*pos(1));
            hess(1,1)=b_*b_*std::cos(a_*pos(0))*std::cos(b_*pos(1));
    ...
    private:
        ...
        complex_t evaluate_pes_c(const CMatrixD1 &pos) const{
            ...
            return complex_t(-std::cos(a_*pos(0).real())*std::cos(b_*pos(1).real()),0.0);
        }
