...
{
    /**
    * \brief This class represents 1D morse potential V(x)=D_e(1-e^{-a(x-x_0)})^2
    *
    ...
    */
	...
	struct Morse_1D
	{
        ...
        Morse_1D()= default;  // default constructor 

        Morse_1D(real_t De, real_t a, real_t x0, const RMatrixDD &mass_inv): De_(De), a_(a), x0_(x0), mass_inv_(mass_inv) {}  //constructor 

        //copy constructor 
        Morse_1D(const Morse_1D &that){
        ...
        //assignment 
        Morse_1D &operator=(const Morse_1D& that){
        ...
        */
    	...
    	{
            return De_*(1.0-std::exp(-a_*(pos(0)-x0_)))*(1.0-std::exp(-a_*(pos(0)-x0_)));
    	}
        ...
        */
    	...
    	RMatrixD1 evaluate_grad(const RMatrixD1& pos) const 
    	{
            ...
            grad(0)= 2.0*a_*De_*(1.0-std::exp(-a_*(pos(0)-x0_)))*std::exp(-a_*(pos(0)-x0_));
    ...
    private:
        ...
        complex_t evaluate_pes_c(const CMatrixD1 &pos) const{
            ...
            return complex_t(De_*(1.0-std::exp(-a_*(pos(0).real()-x0_)))*(1.0-std::exp(-a_*(pos(0).real()-x0_))),0.0);
        }
