...
{
    /**
    * \brief This class represents 2D harmonic oscillator V(x)=0.5*(\sigma_x*x^2+\sigma_y*y^2)
    *
    * \tparam D dimensionality of 2D harmonic oscillator (number of variables)
    */
	...
	struct Harmonic_2D
	{
        ...
        Harmonic_2D()= default;  // default constructor 

        Harmonic_2D(real_t sigmax, real_t sigmay, const RMatrixDD &mass_inv): sigmax_(sigmax), sigmay_(sigmay), mass_inv_(mass_inv) {}  //constructor 

        //copy constructor 
        Harmonic_2D(const Harmonic_2D &that){
        ...
        //assignment 
        Harmonic_2D &operator=(const Harmonic_2D& that){
        ...
        /**
        * \brief calculate the value of 2D harmonic oscillator at position pos
        * \param[in] pos  the position at which we calculate the value of 2D harmonic oscillator
        */

    	real_t evaluate_pes(const RMatrixD1& pos) const 
    	{
            return 0.5*(sigmax_*pos(0)*pos(0)+sigmay_*pos(1)*pos(1));
    	}



        /**
        * \brief calculate the gradient of 2D harmonic oscillator at position pos 
        * \param[in] pos the position at which we calculate the gradient of 2D harmonic oscillator
        */
    	...
    	RMatrixD1 evaluate_grad(const RMatrixD1& pos) const 
    	{
            ...
            grad(0)=sigmax_*pos(0);
            grad(1)=sigmay_*pos(1);

            return grad;
    	}

        /**
        * \brief calculate the hessian of 2D harmonic oscillator at position pos 
        * \param[in] pos the position at which we calculate the 2D harmonic oscillator
        */
